...
class Logger {
 public:
    ...
    inline void setup_corelog(const std::string& filename);

    template <typename... Args>
    void core_log_fmt(LogLevel msg_level, const char* file, int line,
                      const char* func, const char* fmt_str, Args&&... args);

    template <typename... Args>
    void core_log_auto(LogLevel level, const char* file, int line,
                       const char* func, Args&&... args);

    template <typename First, typename... Rest>
    void core_log_dispatch(LogLevel level, const char* file, int line,
                           const char* func, First&& first, Rest&&... rest);
 ...
 private:
    ...
    ~Logger() {
        if (backend_ && backend_->get_is_running_status()) backend_->stop();
    }
